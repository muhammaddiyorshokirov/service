...
import type { AuthContext, JobRow } from "./types.js";
...
function getAuthContext(request: unknown) {
  return (request as { authContext?: AuthContext }).authContext;
}
...
app.addHook("onRequest", async (request, reply) => {
  ...
  (request as { authContext?: AuthContext }).authContext = authContext;
});
...
app.post("/api/media-jobs", async (request, reply) => {
  ...
  let subtitleExt: string | null = null;
  let subtitleOriginalName: string | null = null;
...
function serializeJob(jobId: string, auth: AuthContext) {
...
function serializeJobRecord(job: JobRow) {
  return {
    ...job,
...
function assertJobAccess(auth: AuthContext, job: JobRow) {
...
async function saveMultipartFile(stream: NodeJS.ReadableStream, targetPath: string) {
  await pipeline(stream, createWriteStream(targetPath));
}

function buildR2Prefix(channelName: string | null, contentTitle: string | null, episodeNumber: number) {
