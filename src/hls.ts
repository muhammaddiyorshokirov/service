...
export function buildSubtitleConvertArgs(inputPath: string, outputPath: string) {
  return ["-hide_banner", "-loglevel", "warning", "-i", inputPath, "-y", outputPath];
}

export async function createSubtitlePlaylist(outputDir: string, durationSeconds: number) {
...
export function buildMasterPlaylist(options: {
  ...
  if (options.hasSubtitle) {
    lines.push(
      '#EXT-X-MEDIA:TYPE=SUBTITLES,GROUP-ID="subs",NAME="Default",DEFAULT=YES,AUTOSELECT=YES,LANGUAGE="uz",URI="subtitles/subtitles.m3u8"',
    );
...
export async function parsePlaylistFile(playlistPath: string) {
  const { readFile } = await import("node:fs/promises");
  const raw = await readFile(playlistPath, "utf8");
  return parsePlaylistSegments(raw);
}

export async function writePlaylist(playlistPath: string, segments: PlaylistSegment[]) {
