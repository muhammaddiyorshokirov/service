...
export class SupabaseBridge {
    ...
    async authenticate(accessToken) {
        const { data: { user }, error: authError, } = await this.authClient.auth.getUser(accessToken);
        if (authError || !user) {
    ...
    assertEpisodeAccess(auth, episode) {
        if (auth.isAdmin)
            return;
